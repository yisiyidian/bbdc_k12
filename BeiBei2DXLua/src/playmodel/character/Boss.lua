...
function Boss:createUI(file)
	...
	local ui = cc.Sprite:create("image/playmodel/boss.png")

    local bloodBack = cc.Sprite:create("image/playmodel/back.png")
    bloodBack:setPosition(ui:getContentSize().width /2,ui:getContentSize().height )
    ui:addChild(bloodBack)
    self.showblood = cc.ProgressTimer:create(cc.Sprite:create("image/playmodel/front.png"))
    self.showblood:setPosition(ui:getContentSize().width /2,ui:getContentSize().height )
