...
function MoreInfoChangePwdView:initUI()
	...
	--输入框 旧密码
	local inputOldPwd = InputNode.new("image/signup/shuru_bbchildren_white.png","填写旧密码",nil,nil,nil,nil,true,11)
	inputOldPwd:setPosition(0.5 * s_DESIGN_WIDTH,s_DESIGN_HEIGHT*0.9 - 200)
	self:addChild(inputOldPwd)
	self.inputOldPwd = inputOldPwd
	--输入框 月份
	local inputNewPwd = InputNode.new("image/signup/shuru_bbchildren_white.png","填写新密码",nil,nil,nil,nil,true,11)
	inputNewPwd:setPosition(0.5 * s_DESIGN_WIDTH,s_DESIGN_HEIGHT*0.9 - 330)
	self:addChild(inputNewPwd)
	self.inputNewPwd = inputNewPwd
	--日期
	local inputNewPwdConfirm = InputNode.new("image/signup/shuru_bbchildren_white.png","确认新密码",nil,nil,nil,nil,true,11)
