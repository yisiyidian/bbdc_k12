...
--    scrollView1:addScrollViewEventListener(scrollViewEvent)
    ...
    pageView:setContentSize(cc.size(s_DESIGN_WIDTH,s_DESIGN_HEIGHT))
    pageView:setPosition(0,0)

    for i = 1 , 3 do
        local layout = ccui.Layout:create()
        layout:setContentSize(cc.size(s_DESIGN_WIDTH,s_DESIGN_HEIGHT))

        local imageView = ccui.ImageView:create()
        imageView:setTouchEnabled(true)
        imageView:setScale9Enabled(true)
        imageView:loadTexture("image/PersonalInfo/PLVI/learnedCountBoard_IC.png")
        imageView:setContentSize(cc.size(s_DESIGN_WIDTH,s_DESIGN_HEIGHT))
