...
function HintWord:ctor(time,wordlist)
    ...
    shareBtn.func = function ()
        cx.CXUtils:getInstance():shareURLToWeiXin('http://yisiyidian.com/doubi/html5/index.html?time='..time..'&wordlist='..wordlist, '我完成这些单词只用了'..string.format('%.2f',time)..'秒，你能做到吗？', '贝贝单词－根本停不下来')
        self:removeFromParent()
    end
