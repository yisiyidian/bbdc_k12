...
local SummaryBossLayerIII = class("SummaryBossLayerIII", function ()
...
function SummaryBossLayerIII.create(levelConfig)   
    s_TOUCH_EVENT_BLOCK_LAYER.unlockTouch()
    local layer = SummaryBossLayerIII.new()
                ...
                if selectWord == layer.wordPool[layer.currentIndex][i] then
                    ...
                    if layer.currentBlood <= 0 then
                        ...
                        if killedCrabCount == #layer.wordPool[layer.currentIndex] then
                            ...
                            for m = 1, 5 do
                                for n = 1, 5 do
                                    layer.coconut[m][n]:runAction(cc.Sequence:create(cc.DelayTime:create(0.5),cc.MoveBy:create(0.5,cc.p(0,-s_DESIGN_HEIGHT*0.7))))
                                end
                            end
...
function SummaryBossLayerIII:updateWord(selectStack)
...
function SummaryBossLayerIII:initBossLayer_back(levelConfig)
...
function SummaryBossLayerIII:initBossLayer_girl(levelConfig)
    --add pauseButton
...
function SummaryBossLayerIII:initBossLayer_boss(levelConfig)
...
function SummaryBossLayerIII:initWordList(levelConfig)
...
function SummaryBossLayerIII:initStartIndex(index)
    self.startIndexPool = {}
...
function SummaryBossLayerIII:initMapInfo()
    self.isFirst = {}
    self.isCrab = {}
    self.character = {}
...
function SummaryBossLayerIII:initMap()
    ...
    for i = 1, 5 do
        ...
        for j = 1, 5 do
            ...
            if i == 5 and j == 1 then
                ...
                self.coconut[i][j]:runAction(cc.Sequence:create(cc.DelayTime:create(0.1 * (3 + i - j)),cc.ScaleTo:create(0.1, scale),unlock))
            else
                self.coconut[i][j]:runAction(cc.Sequence:create(cc.DelayTime:create(0.1 * (3 + i - j)),cc.ScaleTo:create(0.1, scale)))
            end
...
function SummaryBossLayerIII:checkTouchLocation(location)
